...
export function AgentCard({ agent, alertCount, onClick }: { agent: Agent; alertCount: number; onClick: () => void }) {
  const s = statusClass[agent.status] ?? statusClass.disconnected;
  const iso = agentIsolation(agent.id);
  const groups = Array.isArray(agent.group) ? agent.group : [];
      ...
      <Card className="hover:border-navy-500 transition-colors">
        ...
        <div className="mt-3 flex items-center justify-between text-xs">
          <div className={cn("inline-flex items-center gap-1.5", s.text)}>
            ...
            {String(agent.status ?? "unknown").replace("_", " ")}
          </div>
          <div className="text-navy-600">{osName} {agent.os?.version ?? ""}</div>
        </div>
        <div className="mt-3 flex items-center justify-between text-xs">
        ...
        <div className="mt-3 flex flex-wrap gap-1">
          {groups.map(g => <span key={g} className="inline-flex items-center h-5 px-1.5 rounded text-[10.5px] bg-navy-200 text-sage border border-navy-400">{g}</span>)}
        </div>
      </Card>
    </button>
  );
