          ...
          { id: "overview", label: "Overview", content: (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div><div className="text-navy-600">Status</div><div className="text-cream">{String(agent.status ?? "unknown").replace("_", " ")}</div></div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(Array.isArray(agent.group) ? agent.group : []).map(g => <span key={g} className="inline-flex items-center h-5 px-1.5 rounded text-[10.5px] bg-navy-200 text-sage border border-navy-400">{g}</span>)}
                </div>
              </div>
            </div>
          )},
