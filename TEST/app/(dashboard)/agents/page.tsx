...
export default function AgentsPage() {
  ...
  const osOptions = useMemo(() => {
    const set = new Set(agents.map(a => a.os?.name ?? "Unknown"));
  ...
  const filtered = useMemo(() => {
    ...
    return agents.filter(a => {
      ...
      if (filters.os !== "all" && (a.os?.name ?? "Unknown") !== filters.os) return false;
      if (q && !(String(a.name ?? "").toLowerCase().includes(q) || String(a.ip ?? "").includes(q) || String(a.os?.name ?? "").toLowerCase().includes(q))) return false;
      return true;
    });
  }, [agents, filters]);
  ...
  const columns: Column<Agent>[] = [
    ...
    { key: "status", header: "Status", cell: a => <Badge tone={a.status === "active" ? "low" : a.status === "disconnected" ? "neutral" : a.status === "pending" ? "medium" : "critical"} dot>{String(a.status ?? "unknown").replace("_", " ")}</Badge> },
    { key: "group", header: "Group", cell: a => <span className="text-sage">{(Array.isArray(a.group) ? a.group : []).join(", ")}</span> },
