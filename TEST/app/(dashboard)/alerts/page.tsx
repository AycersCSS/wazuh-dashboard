...
export default function AlertsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return alerts.filter(a => {
      ...
      const tone = severityBucket(level);
      ...
      if (q && !(String(a.id ?? "").toLowerCase().includes(q) || String(a.rule?.description ?? "").toLowerCase().includes(q) || String(a.agent?.name ?? "").toLowerCase().includes(q))) return false;
      return true;
    });
  }, [alerts, filters, store]);

  const criticalAlertCount = filtered.reduce((n, a) => (Number(a.rule?.level) || 0) >= 13 ? n + 1 : n, 0);
  ...
  const columns: Column<Alert>[] = [
    ...
    },
    {
      ...
      cell: a => <Badge tone={severityBucket(Number(a.rule?.level) || 0)} dot>{severityLabel(Number(a.rule?.level) || 0)} - {Number(a.rule?.level) || 0}</Badge>
    },
    {
