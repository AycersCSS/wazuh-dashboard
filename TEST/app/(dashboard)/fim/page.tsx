...
export default function FimPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return fimEvents.filter(f => {
      ...
      if (q && !(String(f.path ?? "").toLowerCase().includes(q) || String(f.agent ?? "").toLowerCase().includes(q) || String(f.user ?? "").toLowerCase().includes(q))) return false;
      return true;
    });
  ...
  const columns: Column<FimEvent>[] = [
    ...
    { key: "size", header: "Size", cell: f => <span className="font-mono text-sage">{(Number(f.size) || 0).toLocaleString()} B</span> }
  ];
                ...
                <li key={f.id} className="px-4 py-3 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-navy-600 mt-0.5">
                      <span className="font-mono">{f.agent}</span> - <span className="font-mono">{f.user}</span> - {(Number(f.size) || 0).toLocaleString()} B
                    </div>
                  </div>
