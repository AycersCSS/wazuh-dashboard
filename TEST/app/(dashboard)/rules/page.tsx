...
export default function RulesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rules.filter(r => {
      if (q && !(String(r.id).toLowerCase().includes(q) || String(r.description ?? "").toLowerCase().includes(q))) return false;
      return true;
    });
  }, [rules, search]);
  ...
  const columns: Column<Rule>[] = [
    ...
    { key: "groups", header: "Groups", cell: r => <div className="flex flex-wrap gap-1">{(Array.isArray(r.groups) ? r.groups : []).map(g => <span key={g} className="inline-flex items-center h-5 px-1.5 rounded text-[10.5px] bg-navy-200 text-sage border border-navy-400">{g}</span>)}</div> },
    { key: "hits", header: "Hits 24h", width: "120px", sortable: true, sortValue: r => Number(r.hits24h) || 0, cell: r => <span className="font-mono text-cream">{(Number(r.hits24h) || 0).toLocaleString()}</span> },
