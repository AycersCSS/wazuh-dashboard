...
export default function LogsPage() {
  ...
  const rows: Row[] = (data?.entries ?? []).map((e, i) => ({
    id: e?.id ?? `log-${i}`,
    ts: e?.timestamp ?? "",
    source: e?.source ?? "—",
    agent: e?.agent ?? "—",
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      if (q && !(String(r.id).toLowerCase().includes(q) || String(r.message).toLowerCase().includes(q) || String(r.agent).toLowerCase().includes(q))) return false;
      return true;
    });
