    ...
    <div className="fixed inset-0 z-50 animate-slide-up" role="dialog" aria-modal="true" aria-label={typeof title === "string" ? title : undefined}>
      ...
      <div className="relative grid h-full place-items-center p-4">
        <div
          ref={ref}
          tabIndex={-1}
          className={cn("relative w-full bg-white border border-slate-200 rounded-xl shadow-drawer outline-none", w)}
        >
          {(title || subtitle) && (
            <header className="flex items-start justify-between gap-3 p-4 border-b border-slate-200">
              <div>
          ...
          {footer && <footer className="flex items-center justify-end gap-2 p-3 border-t border-slate-200">{footer}</footer>}
        </div>
      </div>
    </div>
  );
...
export function ConfirmDialog({
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <button type="button" className="px-3 h-9 rounded-md bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 text-sm font-medium" onClick={onClose}>Cancel</button>
          <button type="button"
            className={danger ? "px-3 h-9 rounded-md bg-rose-600 text-white hover:bg-rose-700 text-sm font-medium" : "px-3 h-9 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 text-sm font-medium"}
