...
export const Input = forwardRef<HTMLInputElement, Props>(function Input(
  ...
  return (
    <div className="w-full">
      <div className={cn(
        "flex items-center h-9 px-3 bg-navy-100 border rounded-lg",
        error
        ...
        <input
          ref={ref}
          className={cn("flex-1 bg-transparent outline-none text-sm text-cream placeholder:text-navy-600", className)}
          {...rest}
        />
      </div>
      {(error || helper) && <div className={cn("mt-1 text-xs", error ? "text-severity-critical" : "text-navy-600")}>{error ?? helper}</div>}
    </div>
  );
});
